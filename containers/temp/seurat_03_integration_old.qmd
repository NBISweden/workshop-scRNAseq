...
pyenvpath = "/home/jovyan/.pixi/envs/py4r"
```
...
if (fetch_data && !file.exists(path_file)) download.file(url = file.path(path_data, "covid/results_seurat_2026/seurat_covid_qc_dr.rds"), destfile = path_file, method = "curl", extra = curl_upass)
...
wrap_plots(
  DimPlot(alldata, reduction = "pca", group.by = "orig.ident", 
          shuffle = TRUE)+NoAxes()+ggtitle("PCA raw_data"),
  DimPlot(alldata, reduction = "tsne", group.by = "orig.ident", 
          shuffle = TRUE)+NoAxes()+ggtitle("tSNE raw_data"),
  DimPlot(alldata, reduction = "umap", group.by = "orig.ident",
          shuffle = TRUE)+NoAxes()+ggtitle("UMAP raw_data"),
  
  DimPlot(alldata, reduction = "integrated_cca", group.by = "orig.ident", 
          shuffle = TRUE)+NoAxes()+ggtitle("CCA integrated"),
  DimPlot(alldata, reduction = "tsne_cca", group.by = "orig.ident", 
          shuffle = TRUE)+NoAxes()+ggtitle("tSNE integrated"),
  DimPlot(alldata, reduction = "umap_cca", group.by = "orig.ident", 
          shuffle = TRUE)+NoAxes()+ggtitle("UMAP integrated"),
  ncol = 3
...
```
Run `scanorama` via Arrow.

```{r}
#| label: scanorama-define-functions
library(arrow)

# Write scanorama data to Arrow
export_to_arrow <- function(assay_list, gene_list, basedir, prefix) {
  dir_name <- file.path(basedir, paste0("scanorama_data_", prefix))
  dir.create(dir_name, showWarnings = FALSE)
  
  for (i in seq_along(assay_list)) {
    write_ipc_stream(as.data.frame(assay_list[[i]]), sprintf("%s/assay_%d.arrow", dir_name, i))
    write_ipc_stream(data.frame(genes = gene_list[[i]]), sprintf("%s/genes_%d.arrow", dir_name, i))
  }
}

# Read scanorama data from Arrow
import_from_arrow <- function(basedir, prefix, num_datasets) {
  dir_name <- file.path(basedir, paste0("scanorama_data_", prefix))
  integrated_matrices <- list()
  
  for (i in seq_len(num_datasets)) {
    file_path <- sprintf("%s/integrated_%d.arrow", dir_name, i)
    arrow_df <- read_ipc_stream(file_path)
    integrated_matrices[[i]] <- as.matrix(arrow_df)
  }
  
  # Return nested list to match your original downstream code structure
  return(list(integrated_matrices))
}

# Scanorama wrapper
run_scanorama_pipeline <- function(assay_list, gene_list, basedir, prefix, py_env_path) {
  num_ds <- length(assay_list)
  
  # Step A: Export
  cat("Exporting", prefix, "to Arrow...\n")
  export_to_arrow(assay_list, gene_list, basedir, prefix)
  
  # Step B: Call Python natively
  cat("Running Python script...\n")
  system2(
    command = py_env_path,
    args = c("run_scanorama.py", "--basedir", basedir, "--prefix", prefix, "--num_datasets", num_ds)
  )
  
  # Step C: Import
  cat("Importing", prefix, "results back to R...\n")
  result <- import_from_arrow(basedir, prefix, num_ds)
...
At the top of this script, we set the variable `pyenvpath` to point to
...
# integrated.data = basiliskRun(env=pyenvpath, fun=function(datas, genes) {
...
# run scanorama on data via arrow
basedir = "/home/jovyan/work/labs/data"
prefix = "data"

integrated.data <- run_scanorama_pipeline(
  assay_list = assaylist, 
  gene_list = genelist,
  basedir = basedir,
  prefix = prefix,
  py_env_path = pixi_python
)
...
```{r}
#| label: run-scanorama-check-data
# library(arrow)
class(assaylist)
inherits(assaylist[[1]], "sparseMatrix") # Will return FALSE
length(assaylist)
class(assaylist[[1]])
...
length(genelist[[2]])
length(genelist[[8]])
# Check if all sublists are identical to the first one
all_identical <- all(sapply(genelist[-1], function(x) identical(x, genelist[[1]])))
print(all_identical)
```
```{r}
library(arrow)
...
arrow_dir = "/home/jovyan/work/labs/data/scanorama_arrow_data"
dir.create(arrow_dir, showWarnings = FALSE)

# Loop through the 8 datasets
for (i in seq_along(assaylist)) {
  
  # 1. Format the file names (e.g., assay_1.arrow, genes_1.arrow)
  assay_file <- file.path(arrow_dir, sprintf("assay_%d.arrow", i))
  gene_file <- file.path(arrow_dir, sprintf("genes_%d.arrow", i))
  
  # 2. Write the Matrix
  # Arrow requires a data.frame, so we coerce the dense matrix 
  mat_df <- as.data.frame(assaylist[[i]])
  write_ipc_stream(mat_df, assay_file)
  
  # 3. Write the Gene List
  # We wrap the character vector in a data.frame so Arrow can store it as a column
  gene_df <- data.frame(genes = genelist[[i]])
  write_ipc_stream(gene_df, gene_file)
  
  # Optional: Print progress
  cat("Successfully wrote dataset", i, "to Arrow.\n")
}
```
...
system2(
  command = "/home/jovyan/.pixi/envs/py4r/bin/python",
  args = "seurat_03_scanorama.py"
)
```

```{r}
#| label: read-scanorama-integrated-data

library(arrow)

# 1. Initialize an empty list to hold the 8 matrices
integrated_matrices <- list()

# 2. Loop through and read each Arrow file
for (i in 1:8) {
  file_path <- file.path(arrow_dir, sprintf("integrated_%d.arrow", i))
  
  # Read the Arrow IPC file (reads as a data.frame)
  arrow_df <- read_ipc_stream(file_path)
  
  # Convert it to a standard base R matrix
  integrated_matrices[[i]] <- as.matrix(arrow_df)
}

# 3. Recreate the "basilisk" output structure
# We wrap our list of matrices inside another list so that 
# calling `integrated.data[[1]]` works perfectly in your downstream code.
integrated.data <- list(integrated_matrices)

print("Successfully loaded integrated data from Arrow!")
```



``` {r}
#| label: scanorama-create-seurat-object
# # run scanorama via basilisk with assaylis and genelist as input.
# integrated.data = basiliskRun(env=pyenvpath, fun=function(datas, genes) {
#   scanorama <- reticulate::import("scanorama")
#   output <- scanorama$integrate(datasets_full = datas,
...
integrated.counts <- run_scanorama_pipeline(
  assay_list = assaylist, 
  gene_list = genelist,
  basedir = basedir,
  prefix = prefix,
...
intdimred <- do.call(rbind, integrated.counts[[1]])
colnames(intdimred) <- paste0("Scanorama_", 1:100)
rownames(intdimred) <- colnames(alldata)
...
p1 <- DimPlot(alldata, reduction = "umap", group.by = "orig.ident", 
              shuffle = TRUE) + ggtitle("UMAP raw_data")
p2 <- DimPlot(alldata, reduction = "umap_cca", group.by = "orig.ident",
              shuffle = TRUE) + ggtitle("UMAP CCA")
p3 <- DimPlot(alldata, reduction = "umap_harmony", group.by = "orig.ident", 
              shuffle = TRUE) + ggtitle("UMAP Harmony")
p4 <- DimPlot(alldata, reduction = "umap_scanorama", group.by = "orig.ident",
              shuffle = TRUE)+ggtitle("UMAP Scanorama")
