...
quarto_compile() {
    ...
    docker run --rm --platform=linux/amd64 -u root -v "${PWD}:/work" "$DOCKER_SITE" quarto render --profile compile "/work/$LAB_DIR/$1"/*.qmd --to markdown-header_attributes --metadata engine:markdown --log-level warning,error
}
...
qmd_to_ipynb() {
    ...
    for file in "${OUTPUT_DIR}/labs/$1"/*.qmd; do
        docker run --rm --platform=linux/amd64 -u root -v "${PWD}:/work" "$DOCKER_SITE" quarto convert "/work/${file}"
        rm -f "$file"
    done
    shopt -u nullglob
}
