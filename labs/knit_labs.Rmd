...
#PCA_4_SCANPY:The function to plot loading genes only plots genes on the positive axes. Instead plot as a heatmaps, with genes on both postive and negative side, one per pc.
...
#tSNE_ALL2:We can now plot the tSNE colored per dataset. We can start now clearly see the effect of batches present in the dataset.
...
#UMAP_1_SCANPY:The UMAP implementation in SCANPY uses a neighborhood graph as the distance matrix, so we need to first calculate the graph. 
