...
#CT_GSEA7:Read in the gene lists and do some filering.
...
#ST_ALL10:We can use a scRNA-seq dataset as a referenced to predict the proportion of different celltypes in the Visium spots. 
#ST_ALL10:
...
#ST_ALL13:
#ST_ALL13:In this case we will use top DEG genes per cluster, so first we have to run DEG detection on the scRNAseq data.

#ST_R13.1:For SCDC we want to find a we unique markers per cluster, so we select top 20 DEGs per cluster.
