...
ctrl = sce[, sce$orig.ident == 'ctrl_13']
```
...
```{r}
sce <- SetIdent(sce,value = "CCA_snn_res.0.3")
DGE_list <- FindAllMarkers(sce,
                               logfc.threshold = 0,
                               test.use = "wilcox",
                               min.pct = 0,
                               min.diff.pct = 0,
...
sce$predicted <- sce@active.ident

DimPlot(sce,label = T,group.by = "CCA_snn_res.0.3")
DimPlot(sce,label = T)
```
