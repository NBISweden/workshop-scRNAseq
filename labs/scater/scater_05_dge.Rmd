...
markers_genes <- scran::findMarkers( x = sce,
                              groups = as.character(sce$louvain_SNNk10),
                              lfc=.5,
                              pval.type = "all",
                              direction = "up")
...
scater::plotHeatmap(sce[,order(sce$louvain_SNNk10)], features = unique(top5$gene) ,
                    center=T , zlim = c(-3,3) ,
                    colour_columns_by= "louvain_SNNk10",
...
scater::plotExpression(sce, features = unique(top5$gene), x = "louvain_SNNk10", ncol = 5, colour_by = "louvain_SNNk10", scales="free")
```
...
cell_selection <- sce[ , sce$louvain_SNNk10 == 10 ]

#Compute differentiall expression
DGE_cell_selection <- findMarkers( x = cell_selection,
                              groups = cell_selection$type,
                              lfc=.25,
                              pval.type = "all",
                              direction = "any")
...
# Perform enrichment
top_DGE <- DGE_cell_selection$Covid[ (DGE_cell_selection$Covid$p.value < 0.01) & (abs(DGE_cell_selection$Covid$summary.logFC)>0.25), ]
...
gene_rank <- setNames( DGE_cell_selection$Covid$summary.logFC, casefold(rownames(DGE_cell_selection$Covid),upper=T) )
```
