...
As the first step we will fetch the code/scripts for the labs. If you are working on Uppmax, you will work under `r file.path("/proj",ups,"nobackup/username")`. If you are working locally, you can create an empty directory at a suitable location.  

You can either download individual .qmd/.pynb files from the [Contents](/home_contents.html) page or clone the whole repo. If you clone the repo, navigate to `compiled/labs` to work on labs.
