...
if (fetch_data && !file.exists(path_file)) download.file(url = file.path(path_data, "covid/results_bioc_2026/bioc_covid_qc_dr_int.rds"), destfile = path_file, method = "curl", extra = curl_upass)
...
```{r}
#| label: select-tcells
tcells = sce[,sce$leiden_k30 %in% c(1,2)]
