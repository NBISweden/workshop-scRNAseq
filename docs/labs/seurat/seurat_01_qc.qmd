...
df_list = lapply(SplitObject(data.filt, "orig.ident"), function(sub.data.filt){
  ...
  sub.data.filt <- RunUMAP(sub.data.filt, dims = 1:10, verbose = F)
  
  sweep.res <- paramSweep(sub.data.filt)
  sweep.stats <- summarizeSweep(sweep.res, GT = FALSE)
  bcmvn <- find.pK(sweep.stats)
  ...
  sub.data.filt <- doubletFinder(sub.data.filt, pN = 0.25, pK = bcmvn$pK[which.max(bcmvn$BCmetric)], nExp = nExp, PCs = 1:10)
  
  sub.data.filt
})

df = bind_rows(lapply(df_list, function(x){y = x@meta.data[,14, drop = FALSE];colnames(y) = "DF_sep"; y}))
...
table(data.filt$DF_sep, data.filt$DF.classifications_0.25_0.09_297)
DimPlot(data.filt, group.by = c("DF_sep", "DF.classifications_0.25_0.09_297"))
DimPlot(data.filt, group.by = c("DF_sep", "DF.classifications_0.25_0.09_297"), split.by = "orig.ident")
