...
df_list = lapply(SplitObject(data.filt, "orig.ident"), function(sub.data.filt){
  ...
  sub.data.filt <- doubletFinder(sub.data.filt, pN = 0.25, pK = 0.09, #pK = pK[sub.data.filt$orig.ident[1]],
                                 nExp = nExp, PCs = 1:10)
...
df = bind_rows(lapply(df_list, function(x){
  ...
  y = x[, c(grep("^DF\\.class", colnames(x)),grep("^pANN", colnames(x))), drop = FALSE]
