...
if (fetch_data && !file.exists(path_file)) download.file(url = file.path(path_data, "covid/results_seurat_2026/seurat_covid_qc_dr_int.rds"), destfile = path_file, method = "curl", extra = curl_upass)
...
```{r}
#| label: select-tcells
tcells = alldata[,alldata$RNA_snn_res.0.1 %in% c(1,3)]
...
tcells <- FindClusters(tcells, graph.name = "RNA_snn", resolution = 0.25, algorithm = 1, cluster.name = "tcell_0.25")
```
...
wrap_plots(
  ...
  DimPlot(tcells, reduction = "umap_cca", group.by = "tcell_0.25", label = T)+NoAxes()+ggtitle("Full umap, T-cell clust"),
  ...
  DimPlot(tcells, reduction = "umap_tcells", group.by = "tcell_0.25", label = T)+NoAxes()+ggtitle("T-cell umap, T-cell clust"),  
  ncol = 3
) + plot_layout(guides = "collect")
```

As you can see, we do have a new cluster that did not stand out before (cluster 5). But in general the separation looks very similar.
