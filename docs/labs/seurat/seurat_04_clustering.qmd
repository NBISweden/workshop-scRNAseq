...
p1 = DimPlot(alldata, reduction = "umap_cca", group.by = "RNA_snn_res.0.5", label = T) + ggtitle("louvain_0.1")
...
```{r}
#| label: select-tcells
tcells = alldata[,alldata$RNA_snn_res.0.5 %in% c(1,2,4,7)]
...
wrap_plots(
  ...
  DimPlot(tcells, reduction = "umap_tcells", group.by = "tcell_0.5", label = T)+NoAxes()+ggtitle("T-cell umap, T-cell clust"),  
  ncol = 3
