...
if (fetch_data && !file.exists(path_file)) download.file(url = file.path(path_data, "covid/results_seurat_2026/seurat_covid_qc.rds"), destfile = path_file, method = "curl", extra = curl_upass)
...
wrap_plots(
    DimPlot(alldata, reduction = "pca", group.by = "orig.ident", dims = 1:2, shuffle = TRUE),
    DimPlot(alldata, reduction = "pca", group.by = "orig.ident", dims = 3:4, shuffle = TRUE),
    DimPlot(alldata, reduction = "pca", group.by = "orig.ident", dims = 5:6, shuffle = TRUE),
    ncol = 3
) + plot_layout(guides = "collect")
```

To identify which genes contribute the most to each PC, one can retrieve the loading matrix information.
...
wrap_plots(
    DimPlot(alldata, reduction = "umap", group.by = "orig.ident",
            shuffle = TRUE) + ggplot2::ggtitle(label = "UMAP_on_PCA"),
    DimPlot(alldata, reduction = "UMAP10_on_PCA", group.by = "orig.ident", 
            dims = 1:2, shuffle = TRUE) + ggplot2::ggtitle(label = "UMAP10_on_PCA"),
    DimPlot(alldata, reduction = "UMAP10_on_PCA", group.by = "orig.ident", 
            dims = 3:4, shuffle = TRUE) + ggplot2::ggtitle(label = "UMAP10_on_PCA"),
    ncol = 3
) + plot_layout(guides = "collect")
```
...
wrap_plots(
    ...
    DimPlot(alldata, reduction = "tsne", group.by = "orig.ident", shuffle = TRUE),
