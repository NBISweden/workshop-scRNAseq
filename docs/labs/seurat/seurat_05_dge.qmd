...
if (fetch_data && !file.exists(path_file)) download.file(url = file.path(path_data, "covid/results_seurat_2026/seurat_covid_qc_dr_int_cl.rds"), destfile = path_file, method = "curl", extra = curl_upass)
...
wrap_plots(
    ...
    DimPlot(alldata, reduction = "umap_cca", group.by = "orig.ident", shuffle = TRUE) + NoAxes(),
    DimPlot(alldata, reduction = "umap_cca", group.by = "type", shuffle = TRUE) + NoAxes(),
    ncol = 3
)
```
...
Now we can run the function `FindAllMarkers` that will run each of the clusters vs the rest. As you can see, there are some filtering criteria to remove genes that do not have certain log2FC or percent expressed. Here we only test for upregulated genes, so the `only.pos` parameter is set to `TRUE`. 
...
if (!file.exists(genes_file)) download.file(file.path(path_data, "covid/results_seurat_2026/genes_table.csv"), destfile = genes_file,method = "curl", extra = curl_upass)
```
...
tmpfile1 <- file.path(tmpdir, "mast_bayesglm_cl4.Rds")
...
tmpfile2 <- file.path(tmpdir, "mast_glme_cl4.Rds")
