...
Doublet prediction should be run for each dataset separately, so first we need to split the adata object into 8 separate objects, one per sample and then run scrublet on each of them.
