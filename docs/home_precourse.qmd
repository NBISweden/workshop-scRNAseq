...
library(yaml)
library(here)
upc <- yaml::read_yaml(here("docs/_quarto.yml"))$uppmax_compute
ups <- yaml::read_yaml(here("docs/_quarto.yml"))$uppmax_storage
```
...
## {{< fa server >}} Scilifelab Serve 

We will use the Scilifelab Serve compute resources for the workshop. You will need to create an account if you don't already have one. See instructions [here](other/scilifelab-serve.qmd).

## {{< fa brands docker >}} Docker

In addition to ScilifeLab Serve, the lab environments are available as Docker images. These will be useful as backups in case Serve does not work, and will be necessary for the BYOD session. For this, you will need to install Docker: Instructions are [here](other/docker.qmd).
