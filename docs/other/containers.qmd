...
| Scanpy | `ghcr.io/nbisweden/workshop-scrnaseq-scanpy:20260323-2301` | 3.5GB |
...
Inside RStudio, the script `download-labs.sh` is provided that will download the corresponding labs. It creates a `labs` folder with `.qmd` files. In the terminal, run the commands below:
...
::: {.callout-important}
If you are using an Apple Silicon (M-chip) you need to follow the Colima [instructions](https://nbisweden.github.io/workshop-scRNAseq/other/docker.html#running-linux-x86_64-containers-on-apple-silicon)!
:::
...
docker run --platform=linux/amd64 --rm -p 8888:8888 -v ${PWD}:/home/jovyan/work ghcr.io/nbisweden/workshop-scrnaseq-scanpy:20260323-2301
...
Inside JupyterLab, the script `download-labs.sh` is provided that will download the corresponding labs. It creates a `labs` folder with `.ipynb` files. In the terminal, run the commands below:
