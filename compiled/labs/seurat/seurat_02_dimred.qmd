...
reduce the dimentions altering the `n.components` parameter. So here we
will create an UMAP with 10 dimensions.

In Seurat we can add in additional reductions, by default they are named
"pca", "umap", "tsne" etc. depending on the function you run. Here we
will specify an alternative name for the umap with the `reduction.name`
parameter.
