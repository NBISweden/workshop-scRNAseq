...
``` {r}
webpath <- "https://export.uppmax.uu.se/naiss2023-23-3/workshops/scRNAseq/covid_data/"
...
``` {r}
## OBS! Change after new upload link has been provided
indir = "./data/raw"


cov.15 <- Seurat::Read10X_h5(
  filename = file.path(indir,"nCoV_PBMC_15.h5"),
  use.names = T)
cov.1 <- Seurat::Read10X_h5(
  filename = file.path(indir,"nCoV_PBMC_1.h5"),
  use.names = T)
cov.16 <- Seurat::Read10X_h5(
  filename = file.path(indir,"nCoV_PBMC_16.h5"),
  use.names = T)
cov.17 <- Seurat::Read10X_h5(
  filename = file.path(indir,"nCoV_PBMC_17.h5"),
  use.names = T)

ctrl.5 <- Seurat::Read10X_h5(
  filename = file.path(indir,"Normal_PBMC_5.h5"),
  use.names = T)
ctrl.13 <- Seurat::Read10X_h5(
  filename = file.path(indir,"Normal_PBMC_13.h5"),
  use.names = T)
ctrl.14 <- Seurat::Read10X_h5(
  filename = file.path(indir,"Normal_PBMC_14.h5"),
  use.names = T)
ctrl.19 <- Seurat::Read10X_h5(
  filename = file.path(indir,"Normal_PBMC_19.h5"),
  use.names = T)
...
`{r, eval=FALSE} # OBS! Do not run now!  # example method 2: doing it manually total_counts_per_cell <- colSums(alldata@assays$RNA@counts) mito_genes <- rownames(alldata)[grep("^MT-",rownames(alldata))] alldata$percent_mito2 <- colSums(alldata@assays$RNA@counts[mito_genes,]) / total_counts_per_cell`
...
boxplot(as.matrix(t(C[most_expressed, ])), cex = 0.1, las = 1, xlab = "Percent counts per cell",
