...
dataset. We will use the integrated CCA to perform the clustering.
...
p1 = DimPlot(alldata, reduction = "umap_cca", group.by = "RNA_snn_res.0.1", label = T) + ggtitle("louvain_0.5")
...
```
So we can clearly see that clusters 1 and 3 are T-cell clusters, so lets subset the data for those cells
...
Ideally we should rerun all steps of integration with that subset of cells instead of just taking the joint embedding. If you have too few cells per sample in the celltype that you want to cluster it may not be possible.
