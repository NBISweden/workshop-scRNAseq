...
`WhichCells()` to select 32 cells per cluster:

``` {r}
cell_selection <- SetIdent(cell_selection, value = "orig.ident")
sub_data <- subset(cell_selection, cells = WhichCells(cell_selection, downsample = 32))
...
nPatient <- sapply(unique(cell_selection$orig.ident), function(x) {
    rowSums(cell_selection@assays$RNA@layers$counts[, cell_selection$orig.ident
    == x] > 2)
})
...
only significantly up/down regulated genes, but to run GSEA we need statistics 
on a much larger set of genes. This is how it should be run:
