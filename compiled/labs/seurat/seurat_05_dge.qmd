...
markers_genes %>% group_by(cluster)  %>% slice_min(p_val_adj,n=5, with_ties = FALSE) -> top5
...
DGEs are expressed across the individuals within cluster 3:
...
vs 4 samples, we should not expect too many genes with this method.
...
if (file.exists(tmpfile2)) {
    ...
    zlmCond <- suppressMessages(MAST::zlm(~type + nFeature_RNA + (1 | orig.ident), sca, method = "glmer",
