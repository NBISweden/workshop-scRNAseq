...
\`\`\`{r, fig.height = 10, fig.width = 7} #\| label: barplot-markers

par(mfrow = c(2, 5), mar = c(4, 6, 3, 1)) for (i in
unique(top25$cluster)) {  barplot(sort(setNames(top25$avg_log2FC,
top25$gene)[top25$cluster == i\], F), horiz = T, las = 1, main =
paste0(i, " vs. rest"), border = "white", yaxs = "i" ) abline(v = c(0,
0.25), lty = c(1, 2)) }
...
logfc.threshold = -Inf, test.use = "wilcox", min.pct = 0.05,
