...
We also want to keep is the orignial umap for visualization purposes, so
we copy it over to the integrated object.
...
plot_list <- list()
for(i in myfeatures){
  plot_list[[i]] <- FeaturePlot(alldata.int, reduction = "umap",dims = 1:2,
...
This method runs the integration on a dimensionality reduction, in most
