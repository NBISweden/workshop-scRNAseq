...
- *A priori* defined groups or Data-driven groups
  - Differences between samples
  - Differences between clusters
- Simple designs or Complex designs
  - Single samples
  - Sample groups
  - Batch effects


## Simple design: *A priori* groups
...
n <- 30
dfr <- data.frame(sample=rep(c(rep("Timepoint 1",n),rep("Timepoint 2",n)),3),
                  gene=rep(c("CD79A"),each=n*2), norm_expression=c(rnorm(n,3,0.5),rnorm(n,1,0.5)))

ggplot(dfr, aes(x=sample, y=norm_expression))+
  #geom_violin(aes(fill=sample))+
  geom_jitter(aes(color=sample), alpha=0.8)+
  facet_wrap(~gene) + 
  theme(panel.spacing.x = unit(30, "pt"), 
...
- Each cell is a biological replicate
- What are the populations being tested?
...
Be careful when extrapolating from simple designs!


## Simple design: Data-driven groups
...
- C1 vs C2 or C1 vs all
- Often use only positive DEGs
...
Warning: Comparing data-driven clusters will inflate DE analysis ("double-dipping")

DE is helpful to understand why clusters separate, but not to draw novel conclusions about biology
...
- Correcting for covariates (e.g. sex)
...
```{r patients, fig.height=4, fig.width=5}
...
plot_grid(
  ...
  ggplot(cells, aes(x = individual, y = gene, color = group)) + geom_jitter(width = 0.2, alpha = 0.4) + 
    ...
    labs(title = "Pseudobulk", 
         subtitle = "Reduces samples to normalized sums\n(here illustrated with means)") + 
    plottheme,
  align = "hv"
)
...
obj$sample[obj$seurat_clusters == 2] = 2
