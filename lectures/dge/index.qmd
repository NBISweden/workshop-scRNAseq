...
obj = SetIdent(obj, value = "letter.idents")
unique(obj@active.ident)

de_res = FindMarkers(obj, ident.1 = "A", ident.2 = "B")

DimPlot(obj)


VlnPlot(obj, rownames(de_res)[de_res$p_val_adj < 0.05]) & 
  theme(axis.title = element_blank(), 
        plot.title = element_text(size = 10))


DimPlot(obj, group.by = "groups")


set.seed(546021)
obj$mixed = NA
obj$mixed[obj$RNA_snn_res.0.8 == 0] = sample(c("A","A","A","A","B"), sum(obj$RNA_snn_res.0.8 == 0), replace = TRUE)
obj$mixed[obj$RNA_snn_res.0.8 == 1] = sample(c("A","B","B","B","B"), sum(obj$RNA_snn_res.0.8 == 1), replace = TRUE)
obj$mixed[obj$RNA_snn_res.0.8 == 2] = sample(c("A","A","B","B","B"), sum(obj$RNA_snn_res.0.8 == 2), replace = TRUE)
obj$mixed[obj$RNA_snn_res.0.8 == 3] = sample(c("A","B","B","B","B"), sum(obj$RNA_snn_res.0.8 == 3), replace = TRUE)

obj = SetIdent(obj, value = "mixed")
